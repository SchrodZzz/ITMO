package kgeorgiy.jstest.prefix;

import kgeorgiy.jstest.AbstractTests;
import kgeorgiy.jstest.ArithmeticTests;
import kgeorgiy.jstest.Language;
import kgeorgiy.jstest.object.ObjectExpressionTest;
...
public class PrefixSumAvgTest extends PrefixParsingErrorTest {
    ...
    public static final Dialect SUM_ANG_OBJECT = ObjectExpressionTest.ARITHMETIC_OBJECT.copy()
            .rename("sum", "Sum")
            .rename("avg", "Avg");
    ...
    public static class SumAvgTests extends ArithmeticTests {{
        ...
        for (int i = 1; i < 10; i++) {
            final AbstractTests.AbstractExpression[] args = Stream.generate(generator).limit(i).toArray(AbstractTests.AbstractExpression[]::new);
            tests(
                    f("sum", args),
                    f("avg", args)
            );
    ...
    public static void main(final String... args) {
        test(PrefixSumAvgTest.class, PrefixSumAvgTest::new, new SumAvgTests(), args, SUM_ANG_OBJECT, "prefix");
    }
}
