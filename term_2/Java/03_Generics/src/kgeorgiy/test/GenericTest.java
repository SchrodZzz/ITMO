...
public class GenericTest extends BaseTest {
    ...
    public GenericTest() {
        ...
        all(
                ...
                (x, y, z) ->
                        bi(10000000).multiply(bi(x)).multiply(bi(y)).multiply(bi(10000000)).add(bi(z)));
        all(
                "x * y + (z - 1) / 10",
                (x, y, z) -> mul(x, y) + check(z - 1) / 10L,
                (x, y, z) -> x * (double) y + (z - 1.0) / 10,
                (x, y, z) ->
                        bi(x).multiply(bi(y)).add(bi(z).subtract(BigInteger.ONE).divide(BigInteger.TEN)));
    }
    ...
    protected void all(
            final String expression, final F<Long> fi, final F<Double> fd, final F<BigInteger> fbi) {
            ...
            for (int y = y1; y <= y2; y++) {
                for (int z = z1; z <= z2; z++) {
                    ...
                    assert Objects.equals(expected, actual)
                            : String.format(
                            "table[%d][%d][%d](x=%d, y=%d, z=%d]) = %s (expected %s)",
                            x - x1, y - y1, z - z1, x, y, z, actual, expected);
                    counter.passed();
                }
            }
        }
