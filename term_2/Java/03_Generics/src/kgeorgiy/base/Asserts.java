...
public class Asserts {
    ...
    public static void assertEquals(
        ...
        assertTrue(
                String.format("%s:%n     expected `%s`,%n       actual `%s`", message, expected, actual),
    ...
    public static void assertEquals(final String message, final int expected, final int actual) {
        assertTrue(
                String.format("%s: Expected %d, found %d", message, expected, actual), actual == expected);
    }

    public static void assertEquals(
            final String message, final double precision, final double expected, final double actual) {
        assertTrue(
    ...
    public static void checkAssert(final Class<?> c) {
        if (!c.desiredAssertionStatus()) {
            throw new AssertionError(
                    "You should enable assertions by running 'java -ea " + c.getName() + "'");
        }
    }
}
