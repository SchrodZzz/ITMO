...
class ParserGenerator {
    ...
    fun generate(filename: String, outputPath: String = "my_gen") {
        val inputStream = CharStreams.fromFileName(filename)
        val lexer = GrammarGeneratingFileLexer(inputStream)
        val parser = GrammarGeneratingFileParser(CommonTokenStream(lexer))
        ...
        """.trimMargin()
                ...
                rule.variants.forEach { variant ->
                    ...
                    variant.children.forEachIndexed { ind, it ->
                        ...
                        |        ${if (!was) {
                                    was = true; "var "
                                } else ""}tree = $it(${variant.params[ind].replaceAttr()})
                        |        children.add(tree)
                        |        curAttr["$it$ind"] = tree
                        |        ${variant.code[ind].replaceAttr()}
                        |        
