...
class BasicCalculatorParser(private val lexer: BasicCalculatorLexer) {
  ...
  fun t1(acc: Int) : Tree {
    ...
    when (val tokenType = lexer.token.type) {
      TOKEN_TYPE.POW -> {
        if (lexer.token.type != TOKEN_TYPE.POW) {
          throw ParserException("Expected: POW\nActual: ${ lexer.token.type }")
        }
        children.add(lexer.token)
        curAttr["POW0"] = lexer.token
        lexer.nextToken()
        
        var tree = t1(Math.pow(acc.toDouble(), curAttr["f1"]!!.res as Double) as Int)
        children.add(tree)
        curAttr["t11"] = tree
      ...
      TOKEN_TYPE.PLUS, TOKEN_TYPE.MINUS, TOKEN_TYPE.ENDF, TOKEN_TYPE.NUMBER, TOKEN_TYPE.LP, TOKEN_TYPE.RP -> {
        children.add(Token(TOKEN_TYPE.EMPTY, "", null))
         res = acc 
      }
      else -> throw ParserException("Unexpected token ${lexer.token.type}")
