...
public class RxNettyAccountHttpServer {
    ...
    public RxNettyAccountHttpServer(IAccountDao dao) {
        this.dao = dao;
    }
    ...
    private <T> Observable<String> addUser(HttpServerRequest<T> request) {
        Optional<String> error = HttpRequestUtils.checkRequestParameters(request, Collections.singletonList("id"));
        if (error.isPresent()) {
            return Observable.just(error.get());
        }

        long id = HttpRequestUtils.getLongParam(request, "id");
    ...
    private <T> Observable<String> addMoney(HttpServerRequest<T> request) {
        Optional<String> error = HttpRequestUtils.checkRequestParameters(request, Arrays.asList("id", "count"));
        if (error.isPresent()) {
            return Observable.just(error.get());
        }
    ...
    private <T> Observable<String> buyStocks(HttpServerRequest<T> request) {
        Optional<String> error = HttpRequestUtils.checkRequestParameters(request, Arrays.asList("id", "company_name", "count"));
        if (error.isPresent()) {
            return Observable.just(error.get());
        }
